...
@click.command()
@click.option("--input", required=True, help="input hdf5 path")
@click.option("--output", required=True, help="output hdf5 path")
@click.option("--eval_dir", default=None, help="directory to output evaluation metrics")
...
def main(input, output, eval_dir, num_workers):
    ...
    # results = [worker((input, i, do_eval)) for i in range(len(converter))]
    ...
    with h5py.File(output, "r+") as out_file:
        env_meta = FileUtils.get_env_metadata_from_dataset(input)
        abs_env_meta = copy.deepcopy(env_meta)
        abs_env_meta["env_kwargs"]["controller_configs"]["control_delta"] = False
        out_file["data"].attrs["env_args"] = json.dumps(abs_env_meta)
